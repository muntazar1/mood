...
class HackerModeInstaller:
    ...
    def install(self):
        ...
        # move HackerModePro to install path
        if Config.get('actions', 'DEBUG', False):
            print("# can't move the HackerModePro folder ")
        ...
        if os.path.isdir(HACKERMODE_FOLDER_NAME):
            try:
                ...
                self.check()
                print(f'# {GREEN}HackerModePro installed successfully...{NORMAL}')
            ...
            print(f'# try to run tool using\n# {GREEN}"python3 HackerModePro install"{NORMAL}')
            print('# installed failed!')

    def update(self):
        if not Config.get('actions', 'DEBUG', cast=bool, default=False):
            hackermode_command_line_path = os.environ.get("_").split("bin/")[0] + "bin/HackerModePro"
            if os.path.exists(hackermode_command_line_path):
                os.remove(hackermode_command_line_path)
            os.system(
                f'curl -H "Authorization: token {os.environ.get("ACCESS_TOKEN")}" https://raw.githubusercontent.com/Arab-developers/HackerModePro/future/install.sh > HackerModeInstall && bash HackerModeInstall')
            print(f'# {GREEN}HackerModePro updated successfully...{NORMAL}')
        else:
            print("# can't update in the DEUBG mode!")

    def add_shortcut(self):
        # add HackerModePro shortcut...
        try:
            ...
            print(NORMAL + "# add HackerModePro shortcut:")
    ...
    def delete(self, show_message=True):
        ...
        if status in ("y", "yes", "ok", "yep"):
            bin_path = os.path.join(os.environ["SHELL"].split("/bin/")[0], "/bin/HackerModePro")
            tool_path = os.path.join(os.environ["HOME"], ".HackerModePro")
            ...
            if os.path.exists(tool_path):
                ...
                try:
                    ...
                    if show_message:
                        print("# cannot remove HackerModePro shortcut!")
            if show_message:
